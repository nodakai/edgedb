...
impl<'a, 'b> Iterator for &'b mut TokenStream<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.read_token() {
            Ok((token, end)) => Some(Ok(SpannedToken { token, start, end })),
...
impl<'a> TokenStream<'a> {
    ...
    fn read_token(&mut self) -> Result<(Token<'a>, Pos), Error<Token<'a>, Token<'a>>> {
            ...
            '$' => {
                ...
                if let Some((_, c)) = iter.next() {
                    match c {
                        ...
                        _ => return Err(Error::unexpected_static_message("bare $ is not allowed")),
                    }
                } else {
                    return Err(Error::unexpected_static_message("bare $ is not allowed"));
                }
                let end_idx = loop {
                    match iter.next() {
                        Some((end_idx, '$')) => {
...
fn check_prohibited(c: char, escape: bool) -> Result<(), Error<Token<'static>, Token<'static>>> {
    match c {
        '\0' if escape => Err(Error::message_static_message(
