...
use cpython::{ObjectProtocol, PyInt, PyList, PyObject, PyTuple, ToPyObject};
...
use edgeql_parser::tokenizer::{is_keyword, Kind, SpannedToken, TokenStream};
...
pub fn init_module(py: Python) {
    unsafe { TOKENS = Some(Tokens::new(py)) }
}

fn peek_keyword(iter: &mut Peekable<Iter<CowToken>>, kw: &str) -> bool {
    iter.peek()
        .map(|t| {
            (t.kind == Kind::Ident || t.kind == Kind::Keyword) && t.value.eq_ignore_ascii_case(kw)
        })
        .unwrap_or(false)
}
...
fn convert(
    ...
    match token.kind {
        ...
        Eq => Ok((tokens.eq.clone_ref(py), tokens.eq.clone_ref(py), py.None())),
        Ampersand => Ok((
        ...
        At => Ok((tokens.at.clone_ref(py), tokens.at.clone_ref(py), py.None())),
        Argument => {
            if value[1..].starts_with('`') {
                Ok((
                    tokens.argument.clone_ref(py),
                    PyString::new(py, value),
                    PyString::new(py, &value[2..value.len() - 1].replace("``", "`")).into_object(),
                ))
            } else {
                Ok((
                    tokens.argument.clone_ref(py),
                    PyString::new(py, value),
                    PyString::new(py, &value[1..]).into_object(),
                ))
            }
        }
        DecimalConst => Ok((
        ...
        FloatConst => {
            let float_value = float::convert(value)
                .map_err(|msg| TokenizerError::new(py, (&msg, py_pos(py, &token.start))))?;
            Ok((
        ...
        BigIntConst => {
            ...
            let dec_str = dec
                ...
                .ok_or_else(|| {
                    TokenizerError::new(
                        py, // should never happen
                        ("number is not integer", py_pos(py, &token.start)),
                    )
                })?
                .to_str_radix(16);
            Ok((
        ...
        BinStr => Ok((
            ...
            PyBytes::new(
                py,
                &unquote_bytes(value)
                    .map_err(|s| TokenizerError::new(py, (s, py_pos(py, &token.start))))?,
            )
            .into_object(),
        )),
        Str => {
            let content = unquote_string(value)
                .map_err(|s| TokenizerError::new(py, (s.to_string(), py_pos(py, &token.start))))?;
            Ok((
        ...
        BacktickName => Ok((
            ...
            PyString::new(py, value),
            PyString::new(py, &value[1..value.len() - 1].replace("``", "`")).into_object(),
        )),
        Ident | Keyword => {
...
fn complex_bytes() {
    assert_eq!(
        _unquote_bytes(r#"\x09 hello \x0A there"#).unwrap(),
